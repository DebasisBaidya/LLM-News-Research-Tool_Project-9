    ...
    if not st.session_state.authenticated:
        ...
        if st.button("Login"):
            if username == "Debasis" and password == "Baidya123":
                ...
                st.experimental_rerun()  # I’m rerunning the app after login
            else:
...
def generate_summary_and_output():
    ...
    # 📌 Centrally aligned action buttons
    col1, col2 = st.columns(2, gap="large")
    with col1:
        generate = st.button('⚡ Generate Summary', use_container_width=True)  # I’m triggering news search and summary
    with col2:
        reset = st.button("🔄 Reset All", use_container_width=True)  # I’m resetting query and output
    ...
    if generate:
        if query:
            ...
            response = response[:800] + ("..." if len(response) > 800 else "")  # I’m trimming the summary to keep it readable
